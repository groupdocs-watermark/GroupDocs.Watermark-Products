...
############################# Head ############################
head_title: "在 Node.js 中使用高级水印保护照片"
head_description: "使用 Node.js 加快照片水印过程。实施快速、易于使用的编码解决方案。"

############################# Header ############################
title: "在 Node.js via Java 中使用高级水印保护照片" 
description: "使用 Node.js 在照片处理工作流程中部署高级水印策略。我们的工具包支持对 JPG、PNG 和 WEBP 图像文件进行大量自定义。"
...
steps:
    ...
    content: |
      **[GroupDocs.Watermark for Node.js via Java](https://products.groupdocs.com/watermark/nodejs-java/) 增强您的文档安全性：** 将我们的 API 注入您的应用程序，并为许多支持的文件格式生成水印。
      ...
      1. **启动水印：**使用提供我们主要功能的**水印**类开始文档处理。通过将应该由生成的水印保护的 Photo 文件传递给构造函数来对其进行实例化。
...
more_formats:
    enable: true
    title: "JavaScript 为照片添加保护性水印"
    exclude: "PHOTO"
    description: "使用 Node.js 中可自定义的高级水印技术增强照片安全性。在不牺牲质量的前提下保护各种格式的照片，包括 JPG、PNG 和 WEBP。"
    items: 
        # format loop 1
        - name: "水印 PDF"
          format: "PDF"
