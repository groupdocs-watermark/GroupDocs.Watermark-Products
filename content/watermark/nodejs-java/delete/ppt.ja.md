...
steps:
    enable: true
    title: "Node.js via Java による Ppt からウォーターマークを簡単に削除"
    content: |
      **[GroupDocs.Watermark](https://products.groupdocs.com/watermark/nodejs-java/)** は、ビジネス ドキュメントから透かしを削除するプロセスを合理化します。ライブラリをシームレスに統合し、次の簡単な手順に従って、Node.js via Java アプリケーションを強化します。
      ...
      1. Ppt ドキュメントを使用してコア クラス **Watermarker** をインスタンス化することでプロセスを開始します。当社の多用途 API は、ストリームまたはローカル パスとして提供されるかどうかに関係なく、ドキュメントをシームレスに処理します。
    ...
    code:
      ...
      content: |
        ...
        // PPT パスを引数として渡して Watermarker を取得します
