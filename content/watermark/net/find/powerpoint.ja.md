...
steps:
    enable: true
    title: ".NET を使用して Powerpoint ファイル内のウォーターマークを検索"
    content: |
      **[GroupDocs.Watermark](https://products.groupdocs.com/watermark/net/)** を使用すると、ビジネス文書内のウォーターマークを見つけるプロセスが効率化されます。当社のパッケージをお使いの .NET アプリケーションに統合して、その利点を最大限に活用してください。
      
      1. **Watermarker**クラスインスタンスにロードしてください。ファイルパス、ファイルストリーム、またはバイトストリームを指定できます。
      2. **SearchCriteriaオブジェクト**を使用してください。たとえば、画像を指定して類似の透かしを検索できます。テキストウォーターマークが見つかった場合は、テキスト、フォント、色、その他の関連オプションを指定してください。
      3. **Watermarker** オブジェクトの **Search** メソッドを使用して、ドキュメント内に配置されているウォーターマークを取得します。ウォーターマークになりそうなオブジェクトのコレクションが届き、さらに処理を進めることができます。
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // POWERPOINT でテキストウォーターマークを検索

        // POWERPOINT パスのウォーターマーカーを作成
