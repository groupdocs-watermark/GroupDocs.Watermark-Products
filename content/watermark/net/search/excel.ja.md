...
head_title: "C＃、ASP.NET、VB.NETでExcelから透かしを検索"
head_description: ".GroupDocs.Watermark API for .NETを使用してC＃、ASP.NET、VB.NET、および.NETCoreアプリケーション内のスマート検索機能を使用してExcelドキュメントから透かしを検索するNETライブラリ."

title: "C＃.NETでExcelから透かしを検索する"
description: "スマート検索を使用して、C＃、ASP.NET、VB.NET、および.NETCoreアプリケーション内からExcelファイルから可能なすべての透かしを検索します。テキスト、正規表現（RegEx）、画像、ハイパーリンク、文字、およびさまざまな検索オブジェクトに基づいて検索条件を定義し、ソースドキュメントのページ全体または特定のページから透かしを検索します."
...
steps:
    ...
    content_left: |
        ...
        * 力Excelドキュメントで* Watermarker* をインスタンス化します。
    ...
    code: |
        ```cs
        // C＃、ASP.NET、VB.NET、および.NET Coreを使用して、Excelドキュメントで可能な透かしを検索します。
        //入力Excelドキュメントでウォーターマーカーをインスタンス化します
