...
head_title: "C＃、ASP.NET、VB.NETのVSTMから透かしを削除します"
head_description: ".GroupDocs.Watermark API for .NETを使用したC＃、ASP.NET、VB.NET、および.NET Coreアプリケーション内のスマート検索を使用して、VSTMドキュメントから透かしを検索して削除するNETライブラリ."

title: "C＃.NETのVSTMから透かしを削除します"
description: "スマート検索を使用して、C＃、ASP.NET、VB.NET、および.NETCoreアプリケーション内からテキスト形式のVSTMドキュメントから透かしを見つけて削除します。特定のフォント名、色、サイズ、およびその他の一致するプロパティに基づいて透かしを検索および削除するための検索基準を定義します."
...
steps:
    enable: true
    title_left: ".NETのVSTMファイルから透かしを削除します"
    content_left: |
    ...
    code: |
        ```cs
        // C＃、ASP.NET、VB.NET、および.NET CoreアプリケーションのVSTMドキュメントから、テキスト形式の透かしを検索して削除します
