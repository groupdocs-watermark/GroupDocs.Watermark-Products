...
head_title: "C＃、ASP.NET、VB.NETのVSSから透かしを削除します"
head_description: ".GroupDocs.Watermark API for .NETを使用したC＃、ASP.NET、VB.NET、および.NET Coreアプリケーション内のスマート検索を使用して、VSSドキュメントから透かしを検索して削除するNETライブラリ."

title: "C＃.NETのVSSから透かしを削除する"
description: "スマート検索を使用して、C＃、ASP.NET、VB.NET、および.NETCoreアプリケーション内からテキスト形式のVSSドキュメントから透かしを見つけて削除します。特定のフォント名、色、サイズ、およびその他の一致するプロパティに基づいて透かしを検索および削除するための検索基準を定義します."
...
steps:
    enable: true
    title_left: ".NETのVSSファイルから透かしを削除する"
    content_left: |
    ...
    code: |
        ```cs
        // C＃、ASP.NET、VB.NET、および.NET CoreアプリケーションのVSSドキュメントから、テキスト形式の透かしを検索して削除します
