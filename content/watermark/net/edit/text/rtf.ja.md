...
head_title: "C＃、ASP.NET、VB.NETのRTFでテキスト透かしを編集する"
head_description: ".GroupDocs.Watermark API for .NETを使用して、C＃、ASP.NET、VB.NET、および.NETCoreアプリケーションのRTFファイルで見つかったテキスト透かしを編集するためのNETライブラリ."

title: "C＃.NETのRTFでテキスト透かしを編集する"
description: "C＃、ASP.NET、VB.NET、および.NET Coreアプリケーション内でフォーマットされた、RTFドキュメントで見つかったテキスト透かしを検索して変更します。必要に応じて、透かしのサイズ、フォントタイプ、回転角度、およびドキュメントページ上の透かしの位置を管理します。"
...
steps:
    enable: true
    title_left: ".NETのRTFファイルでテキスト透かしを編集する"
    content_left: |
    ...
    code: |
        ```cs
        // C＃、ASP.NET、VB.NET、および.NET Coreアプリケーションで、テキスト透かしを検索してRTFの書式に置き換えます
