...
head_title: "C＃、ASP.NET、VB.NETのスプレッドシートでテキスト透かしを編集する"
head_description: ".GroupDocs.Watermark API for .NETを使用して、C＃、ASP.NET、VB.NET、および.NETCoreアプリケーションのSPREADSHEETファイルで見つかったテキスト透かしを編集するためのNETライブラリ."

title: "C＃.NETのスプレッドシートでテキスト透かしを編集する"
description: "C＃、ASP.NET、VB.NET、および.NET Coreアプリケーション内でフォーマットされた、SPREADSHEETドキュメントで見つかったテキスト透かしを検索して変更します。必要に応じて、透かしのサイズ、フォントタイプ、回転角度、およびドキュメントページ上の透かしの位置を管理します。"
...
steps:
    enable: true
    title_left: ".NETのSPREADSHEETファイルのテキスト透かしを編集します"
    content_left: |
    ...
    code: |
        ```cs
        // C＃、ASP.NET、VB.NET、および.NETCoreアプリケーションのスプレッドシートでテキスト透かしを検索してフォーマットに置き換えます
