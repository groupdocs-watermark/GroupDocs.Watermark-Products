...
head_title: "C＃、ASP.NET、VB.NETのXLSXで画像透かしを編集する"
head_description: ".GroupDocs.Watermark API for .NETを使用して、C＃、ASP.NET、VB.NET、および.NETCoreアプリケーションのXLSXファイルで見つかった画像透かしを編集するためのNETライブラリ."

title: "C＃.NETのXLSXで画像透かしを編集する"
description: "C＃、ASP.NET、VB.NET、および.NET Coreアプリケーション内のXLSXドキュメントで見つかった画像の透かしを検索し、変更します。ドキュメントにBMP、PNG、GIF、JPEG画像の透かしを追加します。また、必要に応じて、透かしのサイズ、フォントタイプ、回転角度、およびドキュメントページ上の透かしの位置を管理します。"
...
steps:
    enable: true
    title_left: ".NETのXLSXファイルで画像透かしを編集する"
    content_left: |
    ...
    code: |
        ```cs
        // C＃、ASP.NET、VB.NET、および.NETCoreアプリケーションのXLSXで画像透かしを検索して置換します
