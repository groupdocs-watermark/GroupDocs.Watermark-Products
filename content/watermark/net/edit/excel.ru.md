...
steps:
    enable: true
    title: "Редактируйте водяные знаки в документах Excel с помощью .NET."
    content: |
      **[GroupDocs.Watermark for .NET](https://products.groupdocs.com/watermark/net/)** позволяет разработчикам .NET легко редактировать водяные знаки в различных документах Excel. Вот упрощенное руководство по использованию нашего API в вашем приложении:
      
      1. Начните с передачи файла Excel в качестве параметра конструктору класса **Watermarker**. Вы можете предоставить файл в виде потока байтов, потока файлов или пути к локальному диску.
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // Редактировать текстовый водяной знак EXCEL

        // Создайте Watermarker, предоставив файл EXCEL.
