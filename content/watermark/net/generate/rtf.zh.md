...
############################# Head ############################
head_title: "RTF 个文档的 C# 水印"
head_description: "将 .NET C# 水印合并到 RTF 文件中，以增强 Word 文档的安全性。"

############################# Header ############################
title: "使用 .NET C# 水印保护 RTF" 
description: "使用 .NET C# 为 RTF 个文件应用安全、可自定义的水印，非常适合在 MS Word 环境中维护文档完整性。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for .NET 为 .NET C# 开发人员配备了复杂的工具，可以有效地向 RTF 文档添加水印。该API可以集成可见和透明的水印，这些水印可以在字体、颜色、大小和位置方面进行自定义，从而确保它们无缝融入文档而不会影响可读性。GroupDocs.Watermark 是法律文档、知识产权文件和敏感通信的理想之选，可提供基本保护，防止未经授权的复制和修改。该工具旨在与 .NET 的所有版本兼容，从而为文档水印提供安全且适应性强的解决方案。

############################# Steps ############################
steps:
    enable: true
    title: "轻松为 Rtf 文档生成水印"
    content: |
      **[GroupDocs.Watermark for .NET](https://products.groupdocs.com/watermark/net/):** .NET 应用程序的高级水印库。及时为您的解决方案提供支持并使用水印保护文档。
      
      1. **核心类：Watermarker。** 我们 API 的主类是 **Watermarker**。您需要在文档处理之前实例化它。不要忘记将 Rtf 文件作为路径或流对象传递给构造函数。
...
more_formats:
    enable: true
    title: "使用 C# 在 RTF 中实现水印"
    exclude: "RTF"
    description: "部署 .NET C# 以在 RTF 个文档中创建强大而微妙的水印，从而增强 Word 文件的安全性和真实性。"
    items: 
        # format loop 1
        - name: "水印 PDF"
          format: "PDF"
