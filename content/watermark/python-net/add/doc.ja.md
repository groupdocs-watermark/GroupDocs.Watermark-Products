...
############################# Head ############################
head_title: "PythonによるDOCファイルの透かし"
head_description: "PythonでDOCファイルに透かしを追加し、Word文書を安全かつ整理された状態に保ちます。"

############################# Header ############################
title: "Python透かしによるDOCの保護" 
description: "Pythonを使ってDOCファイルに安全な透かしを追加します。法的、ビジネス、機密なWord文書に最適です。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for Python via .NETを使って、PythonでDOCファイルに透かしを追加できます。透かしをテキストまたは画像として挿入し、透明度、色、位置を調整し、日付、ロゴ、機密印刷の詳細などを含めることができます。このツールは法的、ビジネス、およびセンシティブな文書に最適で、Python環境でスムーズに動作します。GroupDocs.WatermarkはWordファイルを安全かつ本物に保つ信頼できる方法です。

############################# Steps ############################
steps:
    enable: true
    title: "Docファイルに透かしを素早く追加する"
    content: |
      **[GroupDocs.Watermark for Python via .NET](https://products.groupdocs.com/watermark/python-net/):** あなたの文書に透かしを追加する強力なPythonライブラリです。
      
      1. **メインクラス: Watermarker.** 最初に、**Watermarker**オブジェクトを作成します。Docファイルをファイルパスまたはストリームとして渡して開始します。
...
more_formats:
    enable: true
    title: "PythonのWordファイルに対する透かし"
    exclude: "DOC"
    description: "Pythonを用いてDOCファイルにカスタム透かしを追加し、Word文書を保護および検証します。"
    items: 
        # format loop 1
        - name: "ウォーターマーク PDF"
          format: "PDF"
