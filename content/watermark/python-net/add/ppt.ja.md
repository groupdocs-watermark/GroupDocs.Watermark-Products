...
############################# Head ############################
head_title: "PythonによるPowerPointファイルの透かし"
head_description: "PythonでPowerPointファイルに動的な透かしを追加し、プレゼンテーションを保護します。"

############################# Header ############################
title: "PythonによるPPTの保護" 
description: "Pythonを使ってPowerPointプレゼンテーションにカスタム透かしを追加します。ビジネスや教育に最適です。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for Python via .NETは、PythonでPowerPoint（PPT）ファイルに透かしを追加できるツールです。スライドに目に見えるまたは隠れたマークを追加し、そのスタイル、サイズ、色をカスタマイズします。ビジネスや学校でプレゼンテーションを安全に保つために最適で、GroupDocs.Watermarkはすべての主要なPythonバージョンで動作します。

############################# Steps ############################
steps:
    enable: true
    title: "Pptファイルに透かしを素早く追加する"
    content: |
      **[GroupDocs.Watermark for Python via .NET](https://products.groupdocs.com/watermark/python-net/):** あなたの文書に透かしを追加する強力なPythonライブラリです。
      
      1. **メインクラス: Watermarker.** 最初に、**Watermarker**オブジェクトを作成します。Pptファイルをファイルパスまたはストリームとして渡して開始します。
...
more_formats:
    enable: true
    title: "PythonによるPPTファイルの保護"
    exclude: "PPT"
    description: "PythonでPowerPointファイルに高度な透かしを追加し、不正使用を防ぎ、コンテンツを保護します。"
    items: 
        # format loop 1
        - name: "ウォーターマーク PDF"
          format: "PDF"
