...
############################# Head ############################
head_title: "PythonによるDOCXファイルの透かし"
head_description: "PythonでDOCXファイルに透かしを追加し、Word文書を安全に保ちます。"

############################# Header ############################
title: "Python透かしによるDOCXの保護" 
description: "PythonでDOCXファイルに高度な透かし機能を適用します。どの業界でもWord文書を保護するために最適です。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for Python via .NETは、DOCXファイルに強力な透かし機能を提供します。透かしを自由にスケーリング、移動、ブレンドできるようにします。法的、ビジネス、機密ファイルに使用でき、テキストまたは画像の透かしを自由に選んで配置できます。GroupDocs.Watermarkは最新のPython機能に対応しており、最高レベルの文書保護を実現します。

############################# Steps ############################
steps:
    enable: true
    title: "Docxファイルに透かしを素早く追加する"
    content: |
      **[GroupDocs.Watermark for Python via .NET](https://products.groupdocs.com/watermark/python-net/):** あなたの文書に透かしを追加する強力なPythonライブラリです。
      
      1. **メインクラス: Watermarker.** 最初に、**Watermarker**オブジェクトを作成します。Docxファイルをファイルパスまたはストリームとして渡して開始します。
...
more_formats:
    enable: true
    title: "DOCXに対するPythonによる透かし"
    exclude: "DOCX"
    description: "Pythonを使用してDOCXファイルに安全でカスタムな透かしを追加し、文書の安全性と著作権の管理を向上させます。"
    items: 
        # format loop 1
        - name: "ウォーターマーク PDF"
          format: "PDF"
