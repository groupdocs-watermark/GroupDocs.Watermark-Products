...
############################# Head ############################
head_title: "PythonによるTIFFへの透かし"
head_description: "Pythonを使用してTIFF画像に透かしを追加し、デジタルファイルを保護します。"

############################# Header ############################
title: "PythonによるTIFFの透かし作成" 
description: "PythonでTIFFファイルに強力なカスタム透かしを追加します。デジタルアーカイブやフォトグラファーに最適です。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for Python via .NETは、TIFFファイルに透かしを追加するためのPythonツールキットです。テキストまたは画像の透かしを選択し、透明度、サイズ、および位置を設定し、画像を安全に保ちます。透かしのレイヤリングやスマートフォーマッティングなどの機能を利用すれば、画像コレクションを保護する必要がある人々には欠かせません。

############################# Steps ############################
steps:
    enable: true
    title: "Tiffファイルに透かしを素早く追加する"
    content: |
      **[GroupDocs.Watermark for Python via .NET](https://products.groupdocs.com/watermark/python-net/):** あなたの文書に透かしを追加する強力なPythonライブラリです。
      
      1. **メインクラス: Watermarker.** 最初に、**Watermarker**オブジェクトを作成します。Tiffファイルをファイルパスまたはストリームとして渡して開始します。
...
more_formats:
    enable: true
    title: "PythonによるTIFF画像の保護"
    exclude: "TIFF"
    description: "Pythonを使用してTIFFファイルに透かしを追加および管理し、画像を安全に保ち、コントロール下に置きます。"
    items: 
        # format loop 1
        - name: "ウォーターマーク PDF"
          format: "PDF"
