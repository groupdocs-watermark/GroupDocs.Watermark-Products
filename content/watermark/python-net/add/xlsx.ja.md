...
############################# Head ############################
head_title: "PythonによるXLSXへの透かし"
head_description: "Pythonを使用してXLSXファイルに透かしを追加し、Excel文書を保護します。"

############################# Header ############################
title: "Pythonが提供するXLSXシートの保護" 
description: "PythonでXLSXファイルにカスタム透かしを追加します。ビジネスおよび機密データに最適です。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for Python via .NETは、XLSXファイルに透かしを追加および管理するためのPythonソリューションです。テキストまたは画像の透かしを追加し、ファイルのプロパティに基づいて透かしを自動化し、Excel文書を安全かつプロフェッショナルに保ちます。GroupDocs.Watermarkはすべての最新のPython機能をサポートしています。

############################# Steps ############################
steps:
    enable: true
    title: "Xlsxファイルに透かしを素早く追加する"
    content: |
      **[GroupDocs.Watermark for Python via .NET](https://products.groupdocs.com/watermark/python-net/):** あなたの文書に透かしを追加する強力なPythonライブラリです。
      
      1. **メインクラス: Watermarker.** 最初に、**Watermarker**オブジェクトを作成します。Xlsxファイルをファイルパスまたはストリームとして渡して開始します。
...
more_formats:
    enable: true
    title: "PythonによるXLSXへの透かし"
    exclude: "XLSX"
    description: "Pythonを使用してXLSXファイルに安全で高度な透かしを追加し、Excelデータを保護します。"
    items: 
        # format loop 1
        - name: "ウォーターマーク PDF"
          format: "PDF"
