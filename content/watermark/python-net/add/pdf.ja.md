...
############################# Head ############################
head_title: "PythonによるPDF文書の透かし"
head_description: "PythonでPDFファイルに透かしを追加し、重要な文書を保護します。"

############################# Header ############################
title: "PythonによるPDFの保護" 
description: "Pythonを使用してPDFファイルに透かしを追加します。法的、財務、公式文書に最適です。"
...
about:
    ...
    content: |
       GroupDocs.Watermark for Python via .NETはPythonのAPIで、PDFファイルに透かしを追加します。透かしのテキスト、画像、配置をカスタマイズして任意の文書に対応できます。契約書、報告書、または機密文書を印付けるために使用し、PDFを安全かつプロフェッショナルに保ちましょう。GroupDocs.Watermarkは透明度、回転、およびレイヤー機能に対応し、先進的な透かし機能を提供します。

############################# Steps ############################
steps:
    enable: true
    title: "Pdfファイルに透かしを素早く追加する"
    content: |
      **[GroupDocs.Watermark for Python via .NET](https://products.groupdocs.com/watermark/python-net/):** あなたの文書に透かしを追加する強力なPythonライブラリです。
      
      1. **メインクラス: Watermarker.** 最初に、**Watermarker**オブジェクトを作成します。Pdfファイルをファイルパスまたはストリームとして渡して開始します。
...
more_formats:
    enable: true
    title: "Python透かしによるPDFファイルの保護"
    exclude: "PDF"
    description: "PythonでPDFファイルにカスタム透かしを追加し、機密かつ公式な文書を保護します。"
    items: 
        # format loop 1
        - name: "ウォーターマーク PDF"
          format: "PDF"
