...
head_title: "JavaでWORDから透かしを削除する"
head_description: "GroupDocs.Watermark APIforJavaを使用したJavaアプリケーション内のスマート検索を使用してWORDドキュメントから透かしを検索して削除するJavaライブラリ."

title: "JavaでWORDから透かしを削除する"
description: "スマート検索を使用して、JavaおよびJ2SEアプリケーション内からテキスト形式のWORDドキュメントから透かしを見つけて削除します。特定のフォント名、色、サイズ、およびその他の一致するプロパティに基づいて透かしを検索および削除するための検索基準を定義します."
...
steps:
    enable: true
    title_left: "JavaでWORDファイルから透かしを削除する"
    content_left: |
    ...
    code: |
        ```cs
        //JavaアプリケーションでWORDドキュメントから透かしを検索して削除します
