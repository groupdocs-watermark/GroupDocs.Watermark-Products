...
steps:
    ...
    content: |
      **[GroupDocs.Watermark](https://products.groupdocs.com/watermark/java/)** は、ビジネス ドキュメント内で透かしを見つけるプロセスを簡素化します。このパッケージを Java アプリケーションにインストールして、その利点を活用してください。
      ...
      1. ライブラリ機能を利用するには、Rtf ファイルを **Watermarker** クラスのインスタンスにロードします。ファイル パス、ファイル ストリーム、またはバイト ストリームを指定できます。
    ...
    code:
      ...
      content: |
        ```java {style=abap}
        // RTF ドキュメント内の画像透かしを検索

        // RTF ドキュメントを渡す Watermarker を作成します
