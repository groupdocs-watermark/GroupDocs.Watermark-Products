...
head_title: "JavaのVSXでテキスト透かしを編集する"
head_description: "GroupDocs.Watermark API for Javaを使用して、JavaアプリケーションのVSXファイルで見つかったテキスト透かしを編集するJavaライブラリ."

title: "JavaのVSXでテキスト透かしを編集する"
description: "JavaおよびJ2SEアプリケーション内でフォーマットを使用して、VSXドキュメントで見つかったテキスト透かしを検索および変更します。必要に応じて、透かしのサイズ、フォントタイプ、回転角度、およびドキュメントページ上の透かしの位置を管理します。"
...
steps:
    enable: true
    title_left: "JavaでVSXファイルのテキスト透かしを編集する"
    content_left: |
    ...
    code: |
        ```cs
        //JavaアプリケーションのVSXでフォーマットされたテキスト透かしを検索して更新します
